...
#### 1.1 Measurement Computations for COVID-19 Metrics
...
death_rate_plot <- base_plot + 
  ...
  labs(title = "COVID-19 Death Rate Trend (2020-2024)",
...
cases_plot <- base_plot + 
  ...
  labs(title = "COVID-19 Case Trend (2020-2023)",
       y = "Total COVID Cases")
...
To compare the death rates between each state, we first computed death rates by state in each period and made two different visualisation:

-   **Time Series Plot of State-Level COVID-19 Death Rates**: This graph displays the COVID-19 death rate per 100,000 people for each state over time, using both points and lines to represent changes. This visualization allows for easy comparison of trends between states within the specified period.

<!-- -->

-   **Bar Chart of State-Level Cumulative COVID-19 Death Rates**: This visualization uses a bar chart to display the cumulative COVID-19 death rates per 100,000 people for each state within the specified period. The states are ordered by death rate, facilitating an easy comparison of the pandemic's impact across states. The bars are coloured in light blue for visual clarity, and the states are listed on the y-axis (due to `coord_flip()`), making it straightforward to identify and compare the rates across states.
...
During Period 1, Mississippi State (MS) had the greatest COVID-19 death rate when Vermont (VT) had the lowest.
...
To evaluate the trend of COVID-19 virulence acorss different periods, we first computed case fatality rate (CFR) with the following equation:
...
deaths_pre_2020 <- read_csv("../data/deaths_pre_2020.csv")
...
Will aggregate across all dates for easy visualisation.
