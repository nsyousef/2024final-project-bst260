...
pop_all$"2024" <- ifelse(is.na(pop_all$"2024"),
                         pop_all$"2023",
                         pop_all$"2024") #if population is NA for 2024, use 2023 population
pop_all <- pop_all|>
...
cases <- cases_raw |>
  mutate(cases = parse_number(new_cases),
         ...
         year = year(date),
         week = week(date)) |> # Add week number
...
covid_death_clean <- covid_death|>
  ...
  mutate(covid_death = as.numeric(covid_death),
  ...
  filter(state %in% pop_all$state)|>
  select(state, start_date, end_date, week, covid_death, total_death, year, percentage_expected_death)
